 ...
 * @Update by: LordPsyan
 *
 * @website: https://www.psyanidegaming.com
 *
 * @Discord: https://discord.gg/gFceXEq
 *
 **/
...
{
    if(!sWorld.GetModuleBoolConfig("TeleNPC.Enable", true))
        {
...
class LoadTeleNPCTable : public WorldScript
{
public:
...
class TeleNPC_Announce : public PlayerScript
{
public:

    TeleNPC_Announce() : PlayerScript("TeleNPC_Announce") {}

    void OnLogin(Player* player, bool firstlogin)
    {
        // Announce Module
        if (sWorld.GetModuleBoolConfig("TeleNPC.Enable", true) && sWorld.GetModuleBoolConfig("TeleNPC.Announce", true))
        {
            ChatHandler(player->GetSession()).SendSysMessage("This server is running the |cff4CFF00TeleNPC |rmodule by |cff4CFF00LordPsyan");
        }
    }
};
...
{
    new npc_teleport;
    new LoadTeleNPCTable;
    new TeleNPC_Announce;
}
