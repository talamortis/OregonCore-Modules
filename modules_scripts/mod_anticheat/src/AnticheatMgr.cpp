...
{
	if (!sWorld.GetModuleBoolConfig("Anticheat.DetectJumpHack", true))
		return;
	...
	if (m_Players[key].GetLastOpcode() == MSG_MOVE_JUMP && opcode == MSG_MOVE_JUMP)
	{
		...
		player->TeleportTo(player->GetMapId(), x, y, ground_Z, player->GetOrientation());
...
{
	if (!sWorld.GetModuleBoolConfig("Anticheat.DetectWaterWalkHack", true))
		return;
	...
	if (sWorld.GetModuleBoolConfig("Anticheat.KickPlayerWaterWalkHack", false))
	{
...
{
    if (!sWorld.GetModuleBoolConfig("Anticheat.DetectFlyHack", true))
        return;

    if (player->HasAuraType(SPELL_AURA_FLY) || player->HasAuraType(SPELL_AURA_MOD_INCREASE_MOUNTED_SPEED))
        return;

	/*Thanks to @LilleCarl for info to check extra flag*/
	bool stricterChecks = true;
	if (sWorld.GetModuleBoolConfig("Anticheat.StricterFlyHackCheck", false))
		stricterChecks = !(movementInfo.HasMovementFlag(MOVEMENTFLAG_ASCENDING) && !player->IsInWater());

	if (sWorld.GetModuleBoolConfig("Anticheat.KickPlayerFlyHack", false))
	{
		if (sWorld.GetModuleBoolConfig("Anticheat.WriteLog", false))
...
{
	if (!sWorld.GetModuleBoolConfig("Anticheat.DetectTelePlaneHack", true))
		return;
	...
	if (z_diff > 1.0f)
	{
		...
		// Kill the player
		player->EnvironmentalDamage(DAMAGE_FALL_TO_VOID, player->GetHealth());
...
{
	if (!sWorld.GetModuleBoolConfig("Anticheat.Enabled", 0))
		return;
...
{
	if (!sWorld.GetModuleBoolConfig("Anticheat.DetectClimbHack", false))
		return;
...
{
	if (!sWorld.GetModuleBoolConfig("Anticheat.DetectSpeedHack", true))
		return;
...
void AnticheatMgr::BuildReport(Player* player, uint8 reportType)
{
	...
	if ((uint32)sWorld.GetModuleIntConfig("Anticheat.MaxReportsForDailyReport", 70) < m_Players[key].GetTotalReports())
	{
	...
	/*if (m_Players[key].GetTotalReports() > (uint32)sWorld.GetModuleIntConfig("Anticheat.ReportsForIngameWarnings", 70))
	{
		...
		std::string str = "";
		str = "|cFFFFFC00[Playername:|cFF00FFFF[|cFF60FF00" + std::string(player->GetName()) + "|cFF00FFFF] Possible cheater!";
