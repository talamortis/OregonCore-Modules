...
void buyGuildhouse(Player *player, Creature *_creature, uint32 guildhouseId)
{
 ...
 bool token = sWorld.GetModuleBoolConfig("GuildHouse.TokenOrGold", false);
 int cost = sWorld.GetModuleIntConfig("GuildHouse.BuyCost", 1000);
...
void sellGuildhouse(Player *player, Creature *_creature)
{
 ...
 player->ModifyMoney(sWorld.GetModuleIntConfig("GuildHouse.SellCost", 500) * 10000);

 //display message e.g. "here your money etc."
 char msg[100];
 sprintf(msg, MSG_SOLD, sWorld.GetModuleIntConfig("GuildHouse.SellCost", 500));
 _creature->Whisper(msg, LANG_UNIVERSAL, player);
 }
}
...
class guildmaster_Announce : public PlayerScript
{
public:

    guildmaster_Announce() : PlayerScript("guildmaster_Announce") {}
