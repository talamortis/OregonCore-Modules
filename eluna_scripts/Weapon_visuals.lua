...
About:
All looted weapons have a 25% chance of getting random enchantments
This is pure visual pleasure, the vision will be replaced when the weapon is enchanted.

This script is 100% automatic. You can only put it in your script folder and it will work.
]]
...
local function ONITEMLOOT(event, player, item, count, guid)
    if (item:GetClass() == 2 and SubClasses[item:GetSubClass()]) then
        if (math.random() < chance) then -- 25%被掠夺的武器得到了视觉效果
